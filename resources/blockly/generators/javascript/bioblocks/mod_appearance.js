Blockly.JavaScript['aprnc_create_agent_species4'] = function(block) {
  var number_num_agents = block.getFieldValue('NUM_AGENTS');
  var checkbox_type_visible = block.getFieldValue('TYPE_VISIBLE') == 'TRUE';
  ...
  // Managing agent's visibility on/off code
  var codeIsVisible;

  if(checkbox_type_visible == true)
  {
    codeIsVisible = "set hidden? false\n";
  }
  else if(checkbox_type_visible == false)
  {
    codeIsVisible = "set hidden? true\n";
  }
  ...
  var agentCode = "create-" + variable_name_agent + 'Z' + ' ' + number_num_agents + '[ ' + 
                  "setxy random-xcor random-ycor\n" + codeIsVisible + colorCode + "set nome " + "\"" + 
                  variable_name_agent + "\"" + "\n" + sizeCode + energyCode + "\n" + visionRadCode + "set reprod " + "\"" + 
